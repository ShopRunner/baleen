...
internal class BaleenGeneratorTest {
    ...
    inner class Overrides {
        @Test
        fun `add an override when generating Baleen description`() {
            val schemaStr = """
            {
              "id" : "com.shoprunner.data.Dog",
              "definitions" : {
                "record:com.shoprunner.data.Dog" : {
                  "type" : "object",
                  "additionalProperties" : false,
                  "properties" : {
                    "hasLegs" : {
            ...
            val descriptionStr = """
                ...
                val Dog: DataDescription = describe("Dog", "com.shoprunner.data", "") {
                    it.attr(
                            name = "hasLegs",
            ...
            fun mapBooleanSchemaToStringCoercibleToBoolean(j: BooleanSchema): CodeBlock {
                return CodeBlock.of("%T(%T())", StringCoercibleToBoolean::class, BooleanType::class)
            }

            val booleanTypeOverride = (::mapBooleanSchemaToStringCoercibleToBoolean).asBaleenOverride()

            val outputStream = StringWriter()
            val descriptions = BaleenGenerator.encode(schemaStr.parseJsonSchema(), listOf(booleanTypeOverride))
            descriptions.first().writeTo(outputStream)
            val outputStr = outputStream.toString()
        ...
        @Test
        fun `add an override when generating nullable Baleen description`() {
            val schemaStr = """
                {
            ...
            val descriptionStr = """
                ...
                val Dog: DataDescription = describe("Dog", "com.shoprunner.data.dogs", "") {
                    it.attr(
                        name = "name",
                        type = StringType(),
                        required = true
            ...
            val outputStream = StringWriter()
            val descriptions = BaleenGenerator.encode(schemaStr.parseJsonSchema(), listOf(longTypeOverride))
            descriptions.first().writeTo(outputStream)
            val outputStr = outputStream.toString()
